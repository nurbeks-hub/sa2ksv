...
// ---------------------------------------------------------------- labels + index
const labelsEl = $('#labels'), leadersEl = $('#leaders'), tipEl = $('#tip'), indexEl = $('#index');
...
function buildLabels() {
  for (const id of [...labelIds, ...vesselLabelIds]) {
    ...
    const line = document.createElementNS(SVGNS, 'line'); const dot = document.createElementNS(SVGNS, 'circle'); dot.setAttribute('r', '1.6');
    leadersEl.append(line, dot);
    labelEls[id] = { el, b, s, line, dot, on: 0 };
  }
  for (const p of PARTS) {
    const li = document.createElement('li'); li.dataset.id = p.id; if (p.group === 'vessel') li.className = 'vessel';
    const b = document.createElement('b'); b.textContent = String(p.index).padStart(2, '0');
    const s = document.createElement('span'); li.append(b, s); indexEl.append(li);
    li.addEventListener('mouseenter', () => { S.hoverFromIndex = p.id; });
    li.addEventListener('mouseleave', () => { S.hoverFromIndex = null; });
    li.addEventListener('click', () => inspect(p.id));
  }
}
...
function updateLabels() {
  const showMain = S.explode > 0.82 && S.vessels < 0.4 && !S.inspect && S.intro >= 1;
  const showVessels = S.vessels > 0.6 && !S.inspect && S.intro >= 1;
  const center = toScreen(new THREE.Vector3(0, 0, 0).applyMatrix4(eyeRig.matrixWorld));
  let i = 0;
  const placed = [];
  const hoverId = S.hover || S.hoverFromIndex;
  for (const id in labelEls) {
    ...
    if (PART_BY_ID[id].group === 'muscle' || isV) {
      ...
      dx = 18; dy = up * (58 + (PART_BY_ID[id].index % 3) * 26);
    }
  ...
  for (let a = 0; a < placed.length; a++) {
    for (let b = 0; b < a; b++) {
      ...
      if (overlapX && Math.abs(A.ly - B.ly) < 17) { const dir = A.ly >= A.y ? 1 : -1; A.ly = B.ly + dir * 17; b = -1; }
    }
  }
  for (const P of placed) {
    const { L_, x, y, lx, ly, left } = P;
    L_.el.style.transform = `translate(${lx + (left ? -8 : 8)}px, ${ly - 7}px) translateX(${left ? '-100%' : '0'})`;
  ...
  // index list
  for (const li of indexEl.children) {
    li.classList.toggle('hover', hoverId === li.dataset.id);
    li.classList.toggle('on', S.inspect === li.dataset.id);
  }
  // hover tooltip (not when labels already show that part)
  const tipId = S.hover && !S.dragging ? S.hover : null;
  if (tipId && !(labelEls[tipId]?.el.classList.contains('show'))) {
    const c = CONTENT[tipId]; tipEl.textContent = `${String(PART_BY_ID[tipId].index).padStart(2, '0')}  ${c ? c[S.lang] : tipId}`;
...
function applyLang() {
  ...
  for (const id in labelEls) {
    labelEls[id].b.textContent = String(PART_BY_ID[id].index).padStart(2, '0');
    labelEls[id].s.textContent = CONTENT[id]?.[S.lang] || id;
    labelEls[id].w = 0; // re-measure (text length changed)
  }
  for (const li of indexEl.children) li.querySelector('span').textContent = CONTENT[li.dataset.id]?.[S.lang] || li.dataset.id;
  updateButtons(); updateHint(); updateTourBtn();
...
function updateButtons() {
  const T = UI[S.lang];
  const b = $('#btn-explode'); b.textContent = S.explodeTarget > 0.5 ? T.assemble : T.takeApart; b.classList.toggle('on', S.explodeTarget > 0.5);
  $('#btn-vessels').classList.toggle('on', S.vesselsTarget > 0.5);
}
function updateHint() {
  const T = UI[S.lang];
  const h = $('#hint span');
  const touch = matchMedia('(pointer: coarse)').matches;
  h.textContent = S.vesselsTarget > 0.5 ? T.hintVessels : S.explodeTarget > 0.5 ? T.hintExploded : (touch ? T.hintTouch : T.hint);
}
...
function fillPanel(id) {
  const T = UI[S.lang], c = CONTENT[id] || {}, meta = PART_BY_ID[id];
  const panel = $('#panel');
  panel.querySelector('.idx').textContent = `${String(meta.index).padStart(2, '0')} / ${PARTS.length}`;
  panel.querySelector('.name').textContent = c[S.lang] || id;
  panel.querySelector('.group').textContent = T.groups[meta.group] || meta.group;
  ...
  for (const f of c.facts || []) {
    ...
    const t = document.createElement('div'); t.className = 't'; t.textContent = S.lang === 'kk' ? localNum(f.kk || f.en) : (f[S.lang] || f.en);
    const src = SOURCES[f.src];
    if (src) { const a = document.createElement('a'); a.href = src.url; a.target = '_blank'; a.rel = 'noopener'; a.textContent = `[${f.src + 1}]`; a.title = src.title; t.append(' ', a); }
    li.append(v, t); ul.append(li);
  }
}
function fillAbout() {
  const T = UI[S.lang];
...
function updateTourBtn() { const b = $('#btn-tour'); if (!b) return; b.textContent = tour.on ? UI[S.lang].tourStop : UI[S.lang].tour; b.classList.toggle('on', tour.on); }
...
canvas.addEventListener('pointerdown', e => {
  ...
  S.dragging = true; S.dragDist = 0; canvas.classList.add('dragging');
});
...
canvas.addEventListener('pointerup', e => {
  canvas.classList.remove('dragging'); S.dragging = false;
...
canvas.addEventListener('wheel', e => {
  ...
  S.explodeTarget = THREE.MathUtils.clamp(S.explodeTarget + d * 0.00085, 0, 1);
...
window.addEventListener('keydown', e => {
  ...
  else if (e.key === 'Escape') { if (!$('#aboutbox').hidden) $('#aboutbox').hidden = true; else if (S.inspect) exitInspect(); else if (EMBED) closeEye(); else resetOrbit(); }
...
$('#btn-explode').addEventListener('click', () => { stopTour(); sound.start(); toggleExplode(); });
$('#btn-vessels').addEventListener('click', () => { stopTour(); sound.start(); toggleVessels(); });
...
function tick() {
  ...
  document.body.classList.toggle('show-hint', !S.inspect && S.intro >= 1);
  ...
  const beat = Math.pow(Math.max(0, Math.sin((t * 62 / 60) * Math.PI * 2)), 12);
  stage.bloom.strength = 0.22 + S.vessels * (0.28 + beat * 0.18);
...
window.__eye = {
  ...
  state() { return { embed: EMBED, tour: tour.on, tourT: +tour.t.toFixed(1), explode: S.explode, vessels: S.vessels, inspect: S.inspect, hover: S.hover, pupil: S.pupil, intro: S.intro, lang: S.lang, parts: parts.map(p => p.id), missing: PARTS.filter(p => !partById[p.id]).map(p => p.id) }; },
  info() {
