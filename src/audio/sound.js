// Procedural sound design (Web Audio, no files). Off until the first user gesture (browser autoplay policy).
// Layers: a deep airy drone, a slow heartbeat (a little louder on the vessel layer), a whoosh driven by how
// fast the depth rail is moving (layers dissolving), hover ticks, a focus chime, swells and a morph shimmer.
// Adapted from the author's earlier "The Eye" project.
export class Sound {
  constructor() { this.ctx = null; this.enabled = true; this.morphG = null; this.started = false; this.vessels = 0; this.nextBeat = 0; }
  ...
  start() {
    ...
    // Morph shimmer (♂/♀): a soft rising chord
    this.morphG = ctx.createGain(); this.morphG.gain.value = 0; this.morphG.connect(this.dry);
    for (const f of [329.6, 493.9, 659.3]) { const o = ctx.createOscillator(); o.type = 'sine'; o.frequency.value = f; const og = ctx.createGain(); og.gain.value = 0.03; o.connect(og).connect(this.morphG); o.start(); }
  ...
  morph(level) { if (this.ctx && this.enabled) this.morphG.gain.setTargetAtTime(level, this.ctx.currentTime, 0.15); }
